...
Indeed, as shown in figure~\ref{fig:theory}A and~\ref{fig:theory}B, high negative supercoiling levels, or hypercoiling ($\sigma < \sigma_{basal}$), favor higher expression of bacterial genes, as the thermodynamic reaction of promoter opening required to begin transcription is facilitated~\citep{elhoudaigui2019}.
...
\begin{figure}[H]
  ...
  \caption{\textbf{A}. When DNA is underwound ($\sigma < \sigma_{basal}$, left), gene transcription rates are higher than when DNA is overwound ($\sigma > \sigma_{basal}$, right).
...
Indeed, when an RNA polymerase (RNAP in figure~\ref{fig:theory}C) transcribes a gene, it follows the helical twist of the DNA template, but its rotation is hampered by frictional drag~\citep{ma2016}.
The RNA polymerase therefore acts as a topological barrier and generates an accumulation of positive supercoiling through overwinding of the DNA molecule downstream of the transcribed region, and of negative supercoiling through underwinding upstream of the transcribed region, in what has been called the twin-domain model of DNA supercoiling~\citep{liu1987}, as shown in figure~\ref{fig:theory}C.


\subsection{The Transcription-Supercoiling Coupling}

As transcription is itself regulated by supercoiling (figures~\ref{fig:theory}A and B), these interactions result in a possible dynamic coupling between the transcription levels of neighboring genes, which has been named the transcription-supercoiling coupling~\citep{martisb.2019}.
As previously described, when a gene is transcribed (figure~\ref{fig:theory}C), DNA upstream of the transcribed gene is underwound and the level of negative supercoiling increases ($\sigma$ decreases), while DNA downstream of the transcribed gene is overwound and the level of negative supercoiling decreases ($\sigma$ increases).
...
The influence of this coupling on gene transcription is represented in figure~\ref{fig:theory}D: when two neighboring genes lie in diverging orientations (figure~\ref{fig:theory}D, left), the transcription of each gene generates a local increase in negative supercoiling around the other gene, thereby increasing the transcription level of that gene; each gene therefore reinforces the transcription of the other gene.
Conversely, when two neighboring genes face each other in converging orientations (figure~\ref{fig:theory}D, center), each gene is located downstream of the RNA polymerase during the transcription  of the other gene, leading to a decrease in negative supercoiling and therefore a lower transcription level. In that case, each gene inhibits the other gene.
Finally, if two genes are in a colinear orientation (figure~\ref{fig:theory}D, right), the downstream gene up-regulates the upstream gene, and the upstream gene down-regulates its downstream neighbor.
...
\begin{figure}[H]
  ...
  \caption{Genes along an example genome and local variations in supercoiling (outer ring), and the associated gene interaction network (inner ring).
...
Figure~\ref{fig:network} shows the role played by the transcription-supercoiling coupling in an example genome.
...
Finally, in order to obtain the phenotype of an individual, we numerically compute a solution to the system of equations~\ref{eq:sigma} and~\ref{eq:transcr}, using a fixed point algorithm.
...
Let $f(e_i)$ be the function that computes new supercoiling levels $\sigma'_i$ from $e_i$ using equation~\ref{eq:sigma}, then computes new expression levels $e'_i$ from the new $\sigma'_i$ using equation~\ref{eq:transcr}, and finally returns $e'_i$.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.42\textwidth}
    ...
    \label{subfig:13genes_genome}
  \end{subfigure}
  \begin{subfigure}[t]{0.56\textwidth}
    \includegraphics[width=\textwidth]{alife/img/13genes_expr_level.pdf}
    \label{subfig:13genes_expr}
  \end{subfigure}
  \caption{Left: genome (outer ring) and stable state level of supercoiling $\sigma$ (inner ring) of an example individual with 13 genes in the model.
...
Figure~\ref{fig:13genes} shows the genome (left, outer ring) of an example individual with a genome of 13,000 bp and $n=13$ genes evenly spaced along the genome, and with a basal supercoiling of $\sigma_{basal} = -0.06$.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.42\textwidth}
    ...
    \label{subfig:inversion_genome}
  \end{subfigure}
  \begin{subfigure}[t]{0.56\textwidth}
    \includegraphics[width=\textwidth]{alife/img/inversion_expr_level.pdf}
    \label{subfig:inversion_expr}
  \end{subfigure}
  \caption{Genome, local supercoiling and gene expression levels of a new individual obtained from the individual in figure~\ref{fig:13genes} by switching the positions and orientations of genes B and C.}
  \label{fig:inversion}
\end{figure}

Figure~\ref{fig:inversion} again shows the local supercoiling and gene expression levels of the individual in figure~\ref{fig:13genes}, after reversing the positions and orientations of genes B and C.
...
In order to have environments with distinct effects, we choose a value of $\sigma_A = 0.1$, for which isolated genes are effectively inhibited (as in the top-left panel of figure~\ref{fig:sigma_env}), and a value of $\sigma_B = -0.1$, for which some but not all genes are activated (bottom-left panel).
...
Genomic inversions hence affect gene syntenies and orientations, and therefore affect gene expression levels as presented in subsection~\ref{sec:gene_pos}.
...
\begin{figure}[H]
  ...
  \caption{Result of the inversion of a genomic segment containing genes B and C from the individual presented in figure~\ref{fig:network}.
  The gene interactions which have changed due to the inversion are drawn in red.
  This illustration genome corresponds to the actual individual in our model presented in figure~\ref{fig:inversion}.}
  \label{fig:genome_inv}
\end{figure}

Figure \ref{fig:genome_inv} presents a genome obtained by performing an inversion on the genome shown in figure~\ref{fig:network}.
...
Figure~\ref{fig:mean_activ} summarizes the differences in the proportion of activated genes for each of the three sets of genes, between environments A and B, averaged over the 15 repetitions.
...
\caption{Fraction of activated genes of each type in each environment at the end of the lifecycle, averaged over the best individual in the last generation of each replica.
  ...
  \label{fig:mean_activ}
\end{figure}

Furthermore, in each of the 15 replicates, the fitness of the best individual in the population increases continuously over the course of evolution, as shown in figure~\ref{fig:fitnesses}.
...
\begin{figure}[H]
  ...
  \caption{Evolution of the fitness of the best individual of each replicate at every generation.}
  \label{fig:fitnesses}
\end{figure}

Finally, details of the evolution of one of the 15 replicate populations are shown in figure~\ref{fig:evolution}.
...
\begin{figure}[H]
  ...
  \caption{Number of activated genes of each type and fitness of the best individual at every generation of replicate 13, with a population size of $N=100$, for 250,000 generations.
...
This is shown in more detail in figure~\ref{fig:best_indiv}, which shows the supercoiling level and gene activation levels of the best individual of the last generation of replicate 13, in both environments.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.42\textwidth}
    ...
    \label{subfig:best_genome_A}
  \end{subfigure}
  \begin{subfigure}[t]{0.56\textwidth}
    \includegraphics[width=\textwidth]{alife/img/best_rep13_env_A.pdf}
    \label{subfig:best_expr_A}
  \end{subfigure}
  \vspace{-5mm}

  \begin{subfigure}[t]{0.42\textwidth}
    \includegraphics[width=\textwidth]{alife/img/genome_and_tsc_rep13_env_B.pdf}
    \label{subfig:best_genome_B}
  \end{subfigure}
  \begin{subfigure}[t]{0.56\textwidth}
    \includegraphics[width=\textwidth]{alife/img/best_rep13_env_B.pdf}
    \label{subfig:best_expr_B}
  \end{subfigure}
  \caption{Local supercoiling along the genome and gene transcription levels of the best individual in replicate 13 after 250,000 generations.
  ...
  \label{fig:best_indiv}
\end{figure}

Our experimental results show that, in a model of gene transcription that is structured around the transcription-supercoiling coupling, complex gene interaction networks can evolve.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \label{subfig:param_epsilon_1}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_epsilon-0.01.pdf}
    \label{subfig:param_epsilon_2}
  \end{subfigure}

  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_epsilon.pdf}
    \label{subfig:param_epsilon_3}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_epsilon-0.1.pdf}
    \label{subfig:param_epsilon_4}
  \end{subfigure}
  \caption{Average fraction of activated genes in each environment at the end of evolution, for increasing values of $\epsilon$, from top to bottom and left to right.
  ...
  \label{fig:param_epsilon}
\end{figure}

The results of the additional simulations are presented in figures~\ref{fig:param_epsilon},~\ref{fig:param_c} and~\ref{fig:param_sigma}.
For $\epsilon$, we chose values of $\epsilon = 0.003$, $\epsilon = 0.01$, and $\epsilon = 0.1$, compared to an initial value of $\epsilon = 0.03$, and the results are shown in figure~\ref{fig:param_epsilon}.
...
\begin{figure}[H]
  ...
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \label{subfig:param_c_1}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_inter_coef.pdf}
    \label{subfig:param_c_2}
  \end{subfigure}

  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_inter-coef-1.0.pdf}
    \label{subfig:param_c_3}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_inter-coef-3.0.pdf}
    \label{subfig:param_c_4}
  \end{subfigure}
  \caption{Average fraction of activated genes in each environment at the end of evolution, for increasing values of $c$, from top to bottom and left to right.
...
For $c$, we chose values of $c = 0.1$, $c = 1.0$, and $c = 3.0$, for an initial value of $c = 0.3$, and the results are shown in figure~\ref{fig:param_c}.
...
\begin{figure}[H]
  \begin{subfigure}[t]{0.49\textwidth}
    ...
    \label{subfig:param_sigma_1}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_sigma-0.05.pdf}
    \label{subfig:param_sigma_2}
  \end{subfigure}

  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_sigma.pdf}
    \label{subfig:param_sigma_3}
  \end{subfigure}
  \begin{subfigure}[t]{0.49\textwidth}
    \includegraphics[width=\textwidth]{alife/img/mean_activation_sigma-0.2.pdf}
    \label{subfig:param_sigma_4}
  \end{subfigure}
  \caption{Average fraction of activated genes in each environment at the end of evolution, for more and more distinct environments $\sigma_A$ and $\sigma_B$, from top to bottom and left to right.
